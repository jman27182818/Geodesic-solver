...
const double x0[4] = {0,400,pi/2,0};
...
main (void)
{
  ...
  gsl_odeiv2_system sys = {geodesics, NULL, 8, &mu};
	 ...
	 for(qq = 0; qq < DIM; qq++){
       ...
       for (i = 1; i <= Nsample; i++)
         {
           double taui = -10+(i * tauf / Nsample);
           int status = gsl_odeiv2_driver_apply (d, &tau, fabs(tau-taui), y);
